-- Restore delivery for pending requests created before notification policies
-- were synchronized, and notify technicians when they become available.
    ...
    AND (
      ...
      OR public.distance_km_between(
        ...
        WHEN COALESCE(e.ai_emergency_type, e.clasificacion_ia, 'unknown')
          IN ('Grua / remolque', 'Grúa / remolque', 'tow_service', 'accident')
          THEN 20
        ELSE 15
      END
    )
